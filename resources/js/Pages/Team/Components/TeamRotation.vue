<template>
    <div class="bg-gray-300 dark:bg-gray-800 rounded-lg mb-4 p-4 sm:p-6 h-full shadow-md" :style="{ backgroundColor: $store.state.isDarkMode ? '' : $page.props.auth.team.params.color1 }">
        ...
        <ModalConfirm v-if="confirm" @close="confirm = false" @delete-confirm="confirmDeleteRotation(rotation)" title="Supprimer la rotation" text="Êtes-vous sûr de vouloir supprimer la rotation ?" confirm="Oui, supprimer" cancel="Annuler"></ModalConfirm>
    </div>
</template>
        ...
        confirmDeleteRotation () {
            axios.delete('/team/rotation/' + this.rotation.id)
                .then(response => {
                    ...
                    this.team.rotations = this.team.rotations.filter(item => item.id !== this.rotation.id)
                })
                .catch(error => {
